 ...
 */
async function waitForMainApp(): Promise<boolean> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.log(
        `[STARTUP] Main app returned status ${response.status}, retrying...`
      );
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : 'Unknown error';
  ...
  console.warn(
    '[STARTUP] Main app health check timed out after',
    maxAttempts,
    'attempts'
  );
  return false;
...
const task = cron.schedule(cronExpression, () => {
  ...
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  performSync().catch((error: unknown) => {
    const message = error instanceof Error ? error.message : 'Unknown error';
