...
export const performSync = async () => {
    ...
    try {
        ...
        if (!syncStateResponse.ok) {
            ...
            throw new Error(`Failed to fetch sync state: ${syncStateResponse.statusText}`);
        }
        const responseBody = (await syncStateResponse.json());
        // tRPC with superjson wraps responses in { json: ..., meta: ... }
        // The structure is: { result: { data: { json: <actual data>, meta: {...} } } }
        let syncStateData = null;
        ...
        console.log('[SYNC] Using sync token:', currentSyncToken ? 'yes (incremental)' : 'no (full sync)');
        ...
        console.log('[SYNC] Payload size:', JSON.stringify(syncInput).length, 'bytes');
