...
export const ForumPathBar = (props:ForumPathBarProps) => {
    ...
    console.log(`categoriesInPath length: ${categoriesInPath.length} path length: ${path.length}`);

    const handleForumPathBarClick = ( pathToThisCategory:string, thisCategoryId:number ) => {
        props.router.push({pathname:pathToThisCategory}).then(() => {
    ...
    return (
        <div className="flex flex-row gap-2">
            ...
            { props.categoriesInPath.map((p, i) => {
                ...
                return (
                    <div key={i}>
                            <a href={pathToThisCategory} className="text-white underline decoration-transparent transition duration-300 ease-in-out hover:decoration-inherit" onClick={(e)=>{e.preventDefault();handleForumPathBarClick( pathToThisCategory, p.id )}}>{p.name}</a>
