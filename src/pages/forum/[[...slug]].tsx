...
import { type CategoryWithChildren, type PostWithAuthor, type ThreadWithPostsAndAuthors } from "~/server/api/types";
...
const Forum: NextPage<ForumProps> = ( props:ForumProps ) => {
  const propsObj:CategoryWithChildren[] = superjson.parse(props.categoriesInPath);
  const threadObj:ThreadWithPostsAndAuthors|undefined = props.thread?superjson.parse(props.thread):undefined;
  ...
  const threadsOfCurrentCategoryQuery = api.thread.getThreadsByCategoryId.useQuery({categoryId:currentCategoryId});
  const threadsOfCurrentCategory = threadsOfCurrentCategoryQuery.data || [];

  const rowsOfCurrentCategory = [...childrenOfCurrentCategory, ...threadsOfCurrentCategory];
  ...
  return (
    <>
      <main className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-[#000000] to-[#15162c]">
        ...
        {allCategoriesWithChildrenQuery.isLoading || !props.categoriesInPath || threadsOfCurrentCategoryQuery.isLoading ? (
          <p className="text-white">Ladataan...</p>
        ) : (
          <div className="flex flex-col space-y-6">
            <ForumPathBar router={router} categoriesInPath={propsObj} setCurrentCategoryId={setCurrentCategoryId} setCurrentThreadId={setCurrentThreadId}/>
            <ul className="flex flex-col">
              
              {currentThreadId && threadObj && (
                <div>
                <h2 className="text-white pb-8 text-xl">Lanka: {threadObj.name}</h2>
                <ol>
                    ...
                    {!currentThreadId && rowsOfCurrentCategory.map((row: CategoryWithChildren | Thread) => (
                      <li key={row.id} className="text-white">
                        <ForumRow content={row} router={router} setCurrentCategoryId={setCurrentCategoryId} setCurrentThreadId={setCurrentThreadId} />
                      </li>
                    ))}
                  </ul>
                  </div>
        )}
          { currentThreadId? (<CreatePostBox threadId={currentThreadId}/>) : (
...
const categoryPathExists = async ( path: string[]) : Promise<{categories:CategoryWithChildren[],threadWithPosts:ThreadWithPostsAndAuthors|null}|null> => {
...
export const getServerSideProps = async ( { query } : { query:{slug:string[]} } ) => {
   ...
   if(query.slug) {
    ...
    return { props: { initialCategoryId: categoriesInPath[categoriesInPath.length-1]?.id || 1,
    ...
    const rootCategory = {id:1,name:env.FORUM_ROOT_NAME,parentCategoryId:null,childCategories:[] as Category[]} as CategoryWithChildren;
    const categoriesInPath = [rootCategory] as CategoryWithChildren[];
    return { props: { initialCategoryId: 1, categoriesInPath: superjson.stringify(categoriesInPath) } };
  }
};
